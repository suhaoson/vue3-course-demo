...
// error: const internalModel = reactive<{ [name: string]: unknow }>(props?.model || {});
const internalModel = reactive<{ [name: string]: unknown }>(props?.model || {});
const FormItem = Form.FormItem;
const baseClassName = `${prefixName}-dynamic-form`;

// const onFieldChange = (event: { name: string; value: string | unknow }) = {
...
const emits = defineEmits<{
  (event: "finish", e: unknow): void;
  (event: "finishFail", e: unknow): void;
}>();

const onFinish = (e: unknow) => {
  emits("finish", e);
};

const onFinishFail = (e: unknow) => {
  emits("finishFail", e);
};
